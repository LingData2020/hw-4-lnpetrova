...
```{r}
mono_socio_df %>% pivot_wider(names_from = QuestionCategory, values_from = Responce) -> socmono_wider
```

```{r}
socmono_wider %>% distinct(ParticipantID) %>% count()
socmono_wider %>% group_by(sex) %>% count()
socmono_wider %>% distinct(education)
socmono_wider %>% group_by(education) %>% count()
socmono_wider %>% group_by(handedness) %>% count()
...
My overview is similar to the information provided in Table 2 describing demographic information for bidialectals. However, there are less educational levels mentioned in Table 2 than we have in our dataframe. 
...
library(ggplot2)
 ...
 mono %>% ggplot(aes(x = RT))+ geom_density()
```
...
RT doesn't follow normal distribution because of it's shifted left and has a long right tail.

### 1.3
Normalise data applying the logarithm with base 10 (RTlog = log10(RT)). Use `mutate`.

```{r}
mono_df_new <- mutate(mono, RTlog = log10(RT))
```
...
```{r}
p <- ggplot(mono_df_new, aes(x=RTlog)) +  geom_density()
p
...
In this case, RTlog approximately follows normal distribution but now it's a little bit shifted to the right.
...
```{r}
# hint: sd()
max(mono_df_new$RTlog)
4.886446
min(mono_df_new$RTlog)
1.146128
mean(mono_df_new$RTlog)
3.328846
sd(mono_df_new$RTlog)
0.2076783
median(mono_df_new$RTlog)
3.301573
...
```{r}
ggline(mono1, x = 'AcceptabilityJ', y = 'RTlog', add = 'mean_se', color = 'WordOrder', palette=c('blue', 'purple'))
```
...
+    summarise(RTlog = mean(RTlog)) -> mean_rtlog_long
```
...
I would reject null hypothesis because p-value = 0.007723 that is less than 0,05

What claim about logarithms of reaction time for Congruent and Incongruent stimuli can you make according to this test?

According to this test, logarithms of reaction time for Congruent and Incongruent stimuli are not the same.
...
p-value < 0,05 that means we can reject null hypothesis
...
Both results show that logarithms of reaction time for Congruent and Incongruent stimuli are not the same, but in the second case, p-value is considerably less. 
...
mono_df_new %>% filter(WordOrder == "Incongruent") %>% group_by(ParticipantID,TypeOfStimuli) %>% summarise(RTlog = mean(RTlog)) -> incong_rtlog
...
We can use ANOVA for comparing because there are several types of stimuli (several categories).
...
```
Results:                             Df Sum Sq Mean Sq F value   Pr(>F)    
incong_rtlog$TypeOfStimuli   4  0.833 0.20836   7.868 3.37e-06 ***
Residuals                  663 17.557 0.02648                     
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

As we can see, p-value is really small and less than 0,05 that means statistically significant difference in logarithm of reaction time for different conditions.

Yes, there is statistically significant difference in logarithm of reaction time for different conditions (types of stimuli).
...
Results:
                                                      diff         lwr          upr     p adj
FillerUnacceptable-FillerAcceptable           -0.005617642 -0.06214793  0.050912644 0.9988017
Shape-Color-FillerAcceptable                   0.079992375  0.02510100  0.134883752 0.0007101
Size-Nationality-FillerAcceptable              0.067783558  0.01289218  0.122674935 0.0069086
SubjectiveComment-Material-FillerAcceptable    0.017160151 -0.03773123  0.072051528 0.9129851
Shape-Color-FillerUnacceptable                 0.085610016  0.03071864  0.140501393 0.0002205
Size-Nationality-FillerUnacceptable            0.073401200  0.01850982  0.128292577 0.0025404
SubjectiveComment-Material-FillerUnacceptable  0.022777793 -0.03211358  0.077669170 0.7878859
Size-Nationality-Shape-Color                  -0.012208816 -0.06541082  0.040993189 0.9705996
SubjectiveComment-Material-Shape-Color        -0.062832223 -0.11603423 -0.009630218 0.0113181
SubjectiveComment-Material-Size-Nationality   -0.050623407 -0.10382541  0.002578599 0.0709911

There are several pairs of conditions that have statistically significant difference between logarithms of reaction time (where p-value < 0,05): Shape-Color-FillerAcceptable, Shape-Color-FillerUnacceptable, Size-Nationality-FillerAcceptable, SubjectiveComment-Material-Shape-Color.  
### R code cookbook
